...
- **Tool-smoke harness gained a `--format` layer covering the formatter pipeline (refs #209)** — formatters are a wholly separate subsystem (`getFormattersForFile` → `formatFile`, what `runFormatPhase` drives) that the lint-dispatch path the harness exercised never touched, so ~30 formatters had zero live coverage despite mutating files in place (a silently-broken formatter is higher-stakes than a missed lint). `node scripts/smoke-tools.mjs --format` drives that exact entry per fixture: it asserts the expected smart-default formatter is **selected** for the file and that running it actually **reformats** a deliberately mis-formatted-but-valid fixture (`changed === true`). Validated end-to-end on the dev box for biome (js — the smart-default, not prettier), ruff (py), taplo (toml), shfmt (shell), gofmt, rustfmt, dart, and zig. Wired into the nightly workflow alongside the tool and `--lsp` layers. (No broken formatter found this round — formatters were healthy — but the same bug classes that broke zig-check/elixir-check/#214 now have a guard here too.)
