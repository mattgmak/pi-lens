...
- **Skills resolve under the moved entry** — pi resolves a package's `pi.skills` relative to the **extension entry's directory**, not the package root. Once the entry moved to `./dist/index.js`, `pi.skills: ["./skills"]` resolved to `dist/skills` — which doesn't exist, while the real skills live at the package root, so pi reported a skill path/collision warning. Fixed by setting `pi.skills: ["../skills"]` so it resolves from `dist/` back to the package's root `skills/` (the single, real source). Guarded by `tests/packaging.test.ts` and a CI tarball check that `skills/` ships.
