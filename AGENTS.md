...
**VENDORED grammars — built from source, not downloaded (#423).** A prebuilt grammar wasm can **fatally crash the host runtime** rather than fail gracefully: `tree-sitter-swift.wasm` @ tree-sitter-wasms 0.1.13 triggers a fatal V8 Turboshaft-WASM crash on **Node 24, every OS** (`Fatal process out of memory: Zone`) the first time it's loaded + parsed — uncatchable in-process. Fix = **build the grammar from source** (`tree-sitter build --wasm`, emscripten; the from-source swift wasm survives the guard) and **commit the bytes**. The `VENDORED` map (`download-grammars.ts`) + `vendored` section (`grammars.lock.json`) mark such grammars: they are (1) **excluded from CDN download** (`isVendored` filters the download list; `regenerateManifest` hashes the *committed* bytes, not a fetch), (2) **committed via a `.gitignore` negation** (`/grammars/*` globs contents so `!/grammars/tree-sitter-swift.wasm` + its `.wasm.json` sidecar can be re-included — negating the *dir* wouldn't work), (3) provenance-verified with a **per-grammar `expectedVersion`/`expectedPackage`** override (the swift sidecar records `tree-sitter-swift@0.7.1`, not the global `tree-sitter-wasms@0.1.13`), and (4) **never lazy-fetched** — `VENDORED_GRAMMARS` (`grammar-source.ts`) makes `downloadGrammar` return false, so if the committed bytes are ever absent the client degrades instead of pulling the crashing CDN copy. Guard: `scripts/check-grammar-load.mjs` (`npm run check:grammar-load`) loads + heat-parses each grammar in an **isolated child process** (a crash can't be a caught test failure — it kills the worker); the `.github/workflows/grammar-health.yml` nightly runs it across ubuntu/macOS/windows **and** rebuilds swift from source to catch regressions. To vendor another grammar: add it to `VENDORED` + the lock's `vendored` + `VENDORED_GRAMMARS` + the `.gitignore` negation, build+commit the wasm+sidecar, and `--write-manifest`.
